...
## Скил `planetary-gearbox` — расчёт планетарных коробок

Поверх GEARpie в репозитории есть **вендор-нейтральный скил** для расчёта
многоступенчатых ПЛАНЕТАРНЫХ передач по VDI 2736: `skills/planetary-gearbox/`.
Это переносимый CLI с JSON-контрактом «на входе параметры передачи и ступеней →
на выходе запасы прочности SF/SH со слабым местом, габариты, проверка геометрии».
Физика — пайплайн GEARpie (внешняя пара «солнце–саттелит» через VDI 2736 +
аналитика Lewis/Hertz для внутренней пары «саттелит–венец»). Полный контракт —
в [`skills/planetary-gearbox/SKILL.md`](skills/planetary-gearbox/SKILL.md).

### Установка

Скил импортирует пакет `CLASSES` из корня GEARpie и требует numpy/scipy, поэтому
запускать его нужно **интерпретатором проекта из корня репозитория**:

```bash
# создать venv и поставить зависимости (один раз)
python -m venv .venv
.venv\Scripts\pip install numpy scipy        # Windows
# .venv/bin/pip install numpy scipy          # POSIX
```

Отдельной «установки» скила нет — он работает прямо из дерева репозитория.

### Использование (CLI)

```powershell
# Windows (из корня GEARpie):
.venv\Scripts\python.exe skills\planetary-gearbox\cli.py materials --pretty   # материалы
.venv\Scripts\python.exe skills\planetary-gearbox\cli.py motors    --pretty   # моторы-пресеты
.venv\Scripts\python.exe skills\planetary-gearbox\cli.py calc   --in spec.json --pretty
.venv\Scripts\python.exe skills\planetary-gearbox\cli.py best   --t-out-min 60 --pretty
.venv\Scripts\python.exe skills\planetary-gearbox\cli.py search --i 20 --passing --pretty
```
```bash
# POSIX:
.venv/bin/python skills/planetary-gearbox/cli.py calc --in spec.json --pretty
```

Вывод по умолчанию — JSON (машиночитаемо), флаг `--pretty` даёт сводку для
человека. Результаты расчётов кэшируются в `skills/planetary-gearbox/designs/`
(отслеживается git — там лежат готовые коробки, поиск по ним через `search`/`best`).

### Использование с агентами (Claude, ChatGPT и др.)

Скил — это **локальный CLI**, которому нужны репозиторий + `.venv` (numpy/scipy +
`CLASSES`). Поэтому способ подключения зависит от того, есть ли у агента доступ к
твоей машине:

| Агент | Как использовать |
|---|---|
| **Claude Code** (CLI, локальный shell) | Напрямую: попроси запустить `cli.py …`. Опционально зарегистрировать как Agent Skill — см. ниже. |
| **Любой агент с доступом к shell** | Напрямую через CLI (тот же JSON-контракт). |
| **Claude Desktop** | Песочница не видит локальный репо/venv. Обернуть CLI в **локальный MCP-сервер** и подключить Desktop к нему. |
| **ChatGPT** (веб / Custom GPT) | Так же: песочница Code Interpreter без репо/CLASSES/scipy. Варианты: **Custom GPT Action** к локальному HTTP-серверу-обёртке над CLI; либо ChatGPT по `SKILL.md` генерит `spec.json`/команду, а ты запускаешь её локально и возвращаешь JSON. |

Вендор-нейтральность здесь — это **единый интерфейс** (JSON in/out по `SKILL.md`),
а не «запуск в любом облаке»: движку нужны numpy/scipy и `CLASSES`, поэтому для
облачных ассистентов нужен тонкий локальный мост (MCP-сервер или HTTP-Action).

**Опционально — автообнаружение в Claude Code.** Claude Code ищет скиллы в
`.claude/skills/`. Чтобы не дублировать и не привязывать нейтральный скил к Claude,
сделай локальную ссылку (в коммит не идёт, `.claude/` в `.gitignore`):

```powershell
# Windows (junction, без прав администратора), из корня репо:
cmd /c mklink /J .claude\skills\planetary-gearbox skills\planetary-gearbox
```
```bash
# POSIX:
mkdir -p .claude/skills && ln -s ../../skills/planetary-gearbox .claude/skills/planetary-gearbox
```

После этого Claude Code подхватит `SKILL.md` по описанию автоматически. Канонический
же источник остаётся в нейтральном `skills/planetary-gearbox/` — для ChatGPT и
любых других агентов.
